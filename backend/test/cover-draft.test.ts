...
import type { FileDownload, Files, StoredFile } from '../src/files/application/files'
...
function fixture(download: FileDownload | null = file()) {
  ...
  const files = { privateDownload: async () => download } as unknown as Files
...
test('portada de borrador valida archivo público de campaña, texto alternativo y persistencia', async () => {
  ...
  await assert.rejects(drafts.save('owner', '00000000-0000-4000-8000-000000000001', 'muy corto'), InvalidCampaignCover)
  const saved = await drafts.save('owner', '00000000-0000-4000-8000-000000000001', 'Portada de prueba para el borrador')
  assert.equal(saved.imageUrl, '/api/files/public/00000000-0000-4000-8000-000000000001')
  assert.equal((await drafts.read('owner'))?.altText, 'Portada de prueba para el borrador')
})

test('portada de borrador rechaza documentos privados o archivo ausente', async () => {
  await assert.rejects(fixture(file({ visibility: 'PRIVATE', purpose: 'ORGANIZATION_DOCUMENT' })).save('owner', '00000000-0000-4000-8000-000000000001', 'Portada de prueba para el borrador'), InvalidCampaignCover)
  await assert.rejects(fixture(null).save('owner', '00000000-0000-4000-8000-000000000001', 'Portada de prueba para el borrador'), CampaignCoverUnavailable)
})
